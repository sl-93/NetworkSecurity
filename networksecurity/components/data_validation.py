...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            if set(dataframe.select_dtypes(include=np.number).columns.tolist()) == set(numerical_columns):
                return True
            else:
                return False
        except Exception as e:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if status:
                ...
                if not status:
                    logging.info("The numerical features are different in Train dataframe")

            if status:
                status = self.validate_numerical_columns(test_dataframe)
                if not status:
                    logging.info("The numerical features are different in Test dataframe")
